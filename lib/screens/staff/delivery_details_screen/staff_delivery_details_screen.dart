...
class StaffDeliveryDetailScreen extends StatefulWidget {
  const StaffDeliveryDetailScreen({super.key});

  @override
  State<StaffDeliveryDetailScreen> createState() =>
      _StaffDeliveryDetailScreenState();
}

class _StaffDeliveryDetailScreenState extends State<StaffDeliveryDetailScreen> {
  ...
  Future<void> _loadDeliveriesForDate(DateTime date) async {
    ...
    try {
      String? staffId = _firebaseServices.getCurrentUserID();
      if (staffId != null) {
        ...
        _deliveriesStreamSubscription = _firebaseServices
            .getAllDeliveriesByDateStream(date: date)
            .listen((deliveries) {
